...
Each built-in skill lives in its own subdirectory under `skills/builtin/{core,task,coding}/`, while user-created skills live under `skills/custom/`. The directory contains a `SKILL.md` file that defines the skill's metadata, instructions, work-mode binding, and workflow.

<FileTree>
  <FileTree.Folder name="skills/" defaultOpen>
    <FileTree.Folder name="builtin/" defaultOpen>
      <FileTree.Folder name="core/" defaultOpen>
        <FileTree.Folder name="bootstrap/">
          <FileTree.File name="SKILL.md" />
        </FileTree.Folder>
      </FileTree.Folder>
      <FileTree.Folder name="task/">
        <FileTree.Folder name="deep-research/">
          <FileTree.File name="SKILL.md" />
        </FileTree.Folder>
      </FileTree.Folder>
      <FileTree.Folder name="coding/">
        <FileTree.Folder name="frontend-design/">
          <FileTree.File name="SKILL.md" />
        </FileTree.Folder>
      </FileTree.Folder>
    </FileTree.Folder>
...
The `SKILL.md` file is the authoritative definition of the skill. It is parsed by `skills/parser.py` to extract the skill name, description, work modes, `allowed-tools` restrictions, and supporting resource requirements.

## Built-in skills

KKOCLAW ships with the following public skills:
...
`LocalSkillStorage.load_skills()` scans both `builtin/` and `custom/` directories under the configured skills path, with backward-compatible discovery of the legacy `public/` directory. It re-reads `ExtensionsConfig.from_file()` on every call, which means enabling or disabling a skill through the Gateway API takes effect immediately in the running LangGraph server without a restart.

### Parsing

`parser.py` reads each `SKILL.md` file and extracts structured metadata: name, description, `work_modes`, `allowed-tools`, and any tool or resource requirements.

### Security scanning

`security_scanner.py` checks skill content for potentially dangerous patterns before writes, installs, or edits are accepted. A blocking decision is rejected for manual review. For user-driven `.skill` installs, scanner outages are allowed with a warning; direct writes and edits still fail closed when scanning is unavailable.

### Dependency installation

The `.skill` installer safely extracts the ZIP archive, validates `SKILL.md`, injects missing `work_modes` bindings, and scans `SKILL.md`, `references/`, `templates/`, and `scripts/` files that can enter prompts or execution.
...
- **Through the KKOCLAW App UI**: the skills panel lets you toggle skills on and off.
...
KKOCLAW includes an optional **skill evolution** feature that allows the agent to autonomously create and improve skills in the `skills/custom/` directory:
...
<Cards num={2}>
  <Cards.Card title="Sandbox" href="/en/docs/harness/sandbox" />
  <Cards.Card title="Tools" href="/en/docs/harness/tools" />
</Cards>
