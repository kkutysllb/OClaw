...
`security_scanner.py` checks skill content for potentially dangerous patterns before writes, installs, or edits are accepted. If scanning is unavailable or returns a blocking decision, the write is rejected for manual review.
