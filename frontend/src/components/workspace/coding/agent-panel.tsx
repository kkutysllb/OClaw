...
function AgentPanelInner({ projectId, onThreadIdChange, onFocusFile, onTodosChange }: AgentPanelProps) {
  ...
  // The standalone ``coding_agent`` graph has been retired — the Coding
  // Workbench now talks to the unified Lead Agent with ``work_mode_id``
  // pinned to ``"coding"`` so the Lead Agent's coding branch activates
  // (coding tool set, qiongqi prompt, CodingThreadState). We merge the
  // mode flag into the thread context on every call so it cannot be
  // overridden by stale per-thread settings.
  const codingContext = useMemo(
