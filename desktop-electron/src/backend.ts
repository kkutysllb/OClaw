...
export class BackendManager extends EventEmitter {
   ...
   * `core`, `task`, `coding`. Also creates an empty `custom/` directory
  ...
  private initSkills(): void {
    ...
    for (const bundledRoot of builtinRoots) {
      ...
      // with builtin/core, builtin/task, or builtin/coding. Legacy: ends
      // with public/ (flat).
      const rootBasename = bundledRoot.split(/[\\/]/).pop()!;
      let targetSub: string;
      if (rootBasename === "core" || rootBasename === "task" || rootBasename === "coding") {
