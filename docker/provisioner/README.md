# OClaw Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the OClaw backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `kkoclaw` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `kkoclaw` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `kkoclaw` namespace
     - Create/read/delete Services in the `kkoclaw` namespace
     - Read Namespaces (to create `kkoclaw` if missing)
...
docker compose -p kkoclaw-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
```bash
# Health check
curl http://localhost:9194/health

# Create a sandbox (via provisioner container for internal DNS)
docker exec kkoclaw-provisioner curl -X POST http://localhost:9194/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec kkoclaw-provisioner curl http://localhost:9194/api/sandboxes/test-001

# List all sandboxes
docker exec kkoclaw-provisioner curl http://localhost:9194/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n kkoclaw -l sandbox-id=test-001

# Delete sandbox
docker exec kkoclaw-provisioner curl -X DELETE http://localhost:9194/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec kkoclaw-provisioner curl -s http://localhost:9194/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec kkoclaw-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec kkoclaw-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.kkoclaw/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n kkoclaw`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n kkoclaw`
...
3. **Network Isolation**: Sandbox Pods run in the `kkoclaw` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
