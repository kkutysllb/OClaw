...
start_gateway() {
    ...
    _info "启动 Gateway..."
    :> "$GATEWAY_LOG"
    nohup sh -c "cd backend && KKOCLAW_PROJECT_ROOT=\"$REPO_ROOT\" PYTHONPATH=. .venv/bin/python -m uvicorn app.gateway.app:app --host 0.0.0.0 --port $GATEWAY_PORT $gateway_flags $gateway_worker_flags" \
        > "$GATEWAY_LOG" 2>&1 &
    local gpid=$!
