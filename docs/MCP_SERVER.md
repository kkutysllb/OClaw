...
OClaw 支持可配置的 MCP 服务器和技能来扩展其能力，它们从项目根目录中的专用 `extensions_config.json` 文件加载。
...
对于 `http` 和 `sse` MCP 服务器，OClaw 支持 OAuth token 获取和自动 token 刷新。
...
MCP 服务器公开的工具会在运行时自动发现并集成到 OClaw 的 Agent 系统中。启用后，这些工具无需额外代码变更即可供 Agent 使用。
