# [RFC] 在 OClaw 中增加 `grep` 与 `glob` 文件搜索工具

## Summary

我认为这个方向是对的，而且值得做。

如果 OClaw 想更接近 Claude Code 这类 coding agent 的实际工作流，仅有 `ls` / `read_file` / `write_file` / `str_replace` 还不够。模型在进入修改前，通常还需要两类能力：
...
结论：OClaw 现在缺的不是“再多一个 shell 命令”，而是**文件系统检索层**。
...
- 不把它做成"任意磁盘搜索"，仍然只允许在 OClaw 已授权的路径内执行
...
5. **对 `tool_search` 友好**
   当 OClaw 持续扩展工具集时，`grep` / `glob` 会成为非常高频的基础工具，值得保留为 built-in，而不是让模型总是退回通用 bash。
...
（沙箱重构阶段三后，workspace 已是真实主机绝对路径；上面以 `{KKOCLAW_HOME}/threads/{thread_id}/user-data/workspace` 代表搜索根。）
...
### B. 继续沿用 OClaw 的路径权限模型

这两个工具必须复用当前 `ls` / `read_file` 的路径校验逻辑：

- 本地模式走 `validate_local_tool_path(..., read_only=True)`
- 支持 `/mnt/skills/...`（全局只读技能库，仍为虚拟路径）
- 支持 `/mnt/acp-workspace/...`（ACP 子代理工作区，仍为虚拟路径）
- 支持 thread workspace / uploads / outputs 的真实主机路径解析（`{KKOCLAW_HOME}/threads/{thread_id}/user-data/...`，沙箱重构阶段三后不再有 `/mnt/user-data` 虚拟层）
...
- 在非 local sandbox 场景，优先也通过 OClaw 自己控制的路径访问层实现
...
- 对超大文件直接跳过或只扫前 N KB
- regex 编译失败时返回参数错误
- 输出中的路径直接使用真实主机绝对路径（沙箱重构阶段三后 thread workspace 已是真实路径，Agent 直接使用）；仍需确保不会泄露线程 `user-data/` 根目录之外的宿主路径
...
这会让 OClaw 在代码探索体验上持续落后，也削弱无 bash 或受限 bash 场景下的能力。
...
MCP 可以是补充，但 `glob` / `grep` 作为 OClaw 的基础 coding tool，最好仍然是 built-in，这样才能在默认安装中稳定可用。
...
- 输出仅使用线程 `user-data/` 内的真实主机路径，不泄露该根目录之外的宿主路径
- 大结果集会被截断并明确提示
...
4. 为本地路径校验、真实主机路径解析、结果截断、二进制跳过补测试
...
如果按这个方向做，它会明显提升 OClaw 在 coding / repo exploration 场景下的可用性，而且风险可控。
