...
|---|---|---|---|
| TC-DOCKER-01 | `users.db` volume persistence | Verify the `OClaw_HOME` bind mount survives container restart | needs `docker compose up` |
...
| TC-DOCKER-05 | Admin credentials surfacing | **Updated post-simplify** — was "log scrape", now "0600 credential file in `OClaw_HOME`". The file-based behavior is already validated by TC-1.1 + TC-UPG-13 on sg_dev (non-Docker), so the only Docker-specific gap is verifying the volume mount carries the file out to the host | needs container + host volume |
...
| TC-DOCKER-05 (credential surfacing) | TC-1.1 on sg_dev (file at `~/kk-oclaw/backend/.kkoclaw/admin_initial_credentials.txt`, mode 0600, password 22 chars) — the only Docker-unique step is whether the bind mount projects this path onto the host, which is a `docker compose` config check, not a runtime behavior change |
...
# Optional: pin OClaw_HOME to a stable host path
echo "OClaw_HOME=$HOME/kkoclaw-data" >> .env
```
