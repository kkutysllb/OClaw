...
- `threads.py` - `/api/threads/{id}` - LangGraph 删除后清理本地 OClaw 线程数据
- `artifacts.py` - `/api/threads/{id}/artifacts` - 制品服务
- `suggestions.py` - `/api/threads/{id}/suggestions` - 后续建议生成

Web 对话删除流程现在分布在两个后端面上：LangGraph 处理 `DELETE /api/langgraph/threads/{thread_id}` 以清理线程状态，然后网关的 `threads.py` 路由通过 `Paths.delete_thread_dir()` 移除 OClaw 管理的文件系统数据。
...
│  (packages/harness/kkoclaw/sandbox/local.py) │
│                         │
│  - 单例实例             │
│  - 直接执行             │
│  - 本地环境使用         │
