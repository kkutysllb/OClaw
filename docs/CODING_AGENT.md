...
Coding Agent 是 OClaw 面向真实代码项目的独立 coding 工作台。它不是把普通聊天任务简单接入项目目录，而是在 OClaw 内部新增了一条独立的 Qiongqi 运行边界：独立 session、独立 memory、独立 scratch workspace、独立 skills、独立 diff/review/ROI 事件链路。
...
当前内置 Coding skills 位于 `skills/builtin/coding/`。它们覆盖从需求分析到交付的完整工程链路，例如：
