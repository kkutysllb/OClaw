#!/usr/bin/env python3
"""KKOCLAW Health Check (make doctor).
...
def check_llm_auth(config_path: Path) -> list[CheckResult]:
    ...
    try:
        ...
        for model in data.get("models", []):
            ...
            if use == "kkoclaw.models.openai_codex_provider:CodexChatModel":
            ...
            if use == "kkoclaw.models.claude_provider:ClaudeChatModel":
                credential_paths = [
...
def main() -> int:
    ...
    print()
    print(bold("KKOCLAW Health Check"))
    print("═" * 40)
    ...
    if total_fails == 0 and total_warns == 0:
        ...
        print(f"Run {cyan('make dev')} to start KKOCLAW")
    elif total_fails == 0:
