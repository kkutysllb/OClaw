...
scoped by ``(user_id, server_name, scope_key)`` — typically scope_key is the
thread_id — so that consecutive tool calls share the same session and
server-side state. Sessions are evicted in LRU order when the pool reaches
capacity.

The ``user_id`` dimension ensures that two users who each configure a server
with the same name but different credentials (API keys, URLs) do not share a
pooled session.
...
class MCPSessionPool:
    """Manages persistent MCP sessions scoped by ``(user_id, server_name, scope_key)``.

    The ``user_id`` dimension ensures that two users who each configure a
    server with the same name but different credentials (API keys, URLs)
    do not share a pooled session.
    """
    ...
    def __init__(self) -> None:
        ...
        # In-flight creations, keyed by (user_id, server, scope). Lets concurrent
        # callers on the same loop share a single creation instead of each
        # spawning a duplicate session. Value: (loop, ready_future, owner_task, close_event).
        self._inflight: dict[
            tuple[str, str, str],
            tuple[
                asyncio.AbstractEventLoop,
    ...
    async def get_session(
        ...
        *,
        ...
        Args:
            ...
            user_id: User bucket so that per-user servers with the same
                name but different credentials don't share a session.

        Returns:
            An initialized ``ClientSession``.
        """
        key = (user_id, server_name, scope_key)
        ...
        logger.info("Created persistent MCP session for user=%s/%s/%s", user_id, server_name, scope_key)
        return session
    ...
    async def close_scope(self, scope_key: str) -> None:
        ...
        with self._lock:
            keys = [k for k in self._entries if k[2] == scope_key]
            entries = [(self._entries.pop(k)) for k in keys]
            inflight_keys = [k for k in self._inflight if k[2] == scope_key]
    ...
    async def close_server(self, server_name: str, *, user_id: str | None = None) -> None:
        """Close all sessions for a given server.

        If *user_id* is provided, only sessions for that user are closed.
        """
        with self._lock:
            if user_id is not None:
                keys = [k for k in self._entries if k[1] == server_name and k[0] == user_id]
                inflight_keys = [k for k in self._inflight if k[1] == server_name and k[0] == user_id]
            else:
                keys = [k for k in self._entries if k[1] == server_name]
                inflight_keys = [k for k in self._inflight if k[1] == server_name]
            entries = [(self._entries.pop(k)) for k in keys]
            inflight = [self._inflight.pop(k) for k in inflight_keys]
        for _session, loop, task, close_evt in entries:
            await self._shutdown_entry(loop, task, close_evt)
    ...
    async def close_user(self, user_id: str) -> None:
        """Close all sessions belonging to *user_id* (all servers, all scopes).

        Called when a user's MCP config is invalidated so that pooled
        sessions created with the old credentials are torn down.
        """
        with self._lock:
            keys = [k for k in self._entries if k[0] == user_id]
            entries = [(self._entries.pop(k)) for k in keys]
            inflight_keys = [k for k in self._inflight if k[0] == user_id]
            inflight = [self._inflight.pop(k) for k in inflight_keys]
        for _session, loop, task, close_evt in entries:
            await self._shutdown_entry(loop, task, close_evt)
        for loop, _ready, task, close_evt in inflight:
