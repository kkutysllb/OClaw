...
from kkoclaw.config.paths import VIRTUAL_PATH_PREFIX, Paths, get_paths
from kkoclaw.mcp.client import build_servers_config
from kkoclaw.mcp.oauth import build_oauth_tool_interceptor, get_initial_oauth_headers
...
def _local_uri_to_virtual_path(
    ...
    *,
    ...
    only thing missing is the virtual prefix the rest of the engine addresses them
...
def _make_session_pool_tool(
    ...
    *,
    ...
    by ``(user_id, server_name, thread_id)``.  This ensures stateful MCP
    servers (e.g. Playwright) keep their state across tool calls within the
    same thread, while keeping per-user sessions separate.
    ...
    async def call_with_persistent_session(
        ...
        **arguments: Any,
        ...
        effective_user_id = resolve_runtime_user_id(runtime) if user_id == "default" else user_id
        ...
        if is_stdio:
            ...
            source_base_dir, tmp_dir, before_files = await asyncio.to_thread(
                _prepare_stdio_workspace, paths, thread_id=thread_id, user_id=effective_user_id
            )
        ...
        session = await pool.get_session(server_name, thread_id, session_connection, user_id=effective_user_id)
...
async def get_mcp_tools(
    ...
    *,
    ...
    Args:
        extensions_config: The resolved config to load tools from.  When
            ``None`` the global ``extensions_config.json`` is read
            (legacy/DEFAULT_USER_ID behaviour).
        user_id: Used to scope pooled sessions so that different users'
            servers don't share sessions.
    ...
    # Use the provided config, or fall back to the global file.
            ...
            for tool in server_tools:
                ...
                if tool.name.startswith(f"{source_name}_") and transport == "stdio":
                    wrapped_tools.append(
                        _make_session_pool_tool(tool, source_name, servers_config[source_name], tool_interceptors, user_id=user_id)
                    )
                else:
                    wrapped_tools.append(tool)

        # Patch tools to support sync invocation, as kkoclaw client streams synchronously
