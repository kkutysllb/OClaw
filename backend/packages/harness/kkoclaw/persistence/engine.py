...
async def init_engine(
    ...
    *,
    ...
    try:
        ...
        logger.debug("kkoclaw.persistence.models not found; skipping auto-create tables")
...
async def init_engine_from_config(config) -> None:
    ...
    if config.backend == "memory":
        ...
        return
    # Anchor the directory to runtime_home via _resolved_sqlite_dir, matching
    # the url path below — otherwise ``os.makedirs(sqldir)`` on a relative
    # config value would create ``<CWD>/.kkoclaw/data/`` in the working tree
    # even though the engine itself writes to ``<runtime_home>/data/``.
    sqlite_dir = (
        config._resolved_sqlite_dir if config.backend == "sqlite" else ""
    )
    await init_engine(
