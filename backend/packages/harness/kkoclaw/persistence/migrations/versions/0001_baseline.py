...
def upgrade() -> None:
    ...
    Idempotent: every ``op.create_table`` / ``op.create_index`` block is
    guarded by an existence check so this revision is a no-op on a DB whose
    baseline tables already exist. OClaw provisions baseline tables via
    ``Base.metadata.create_all`` at engine init, so an existing OClaw DB will
    have all these tables already; re-running this migration (e.g. an
    un-stamped DB upgraded from ``base``) must not raise "table already
    exists". On a fresh DB none of the tables exist yet, so this runs the
    full DDL exactly as upstream's autogenerate produced it.
    """
    bind = op.get_bind()
    existing_tables = set(sa.inspect(bind).get_table_names())

    if "channel_connections" not in existing_tables:
        op.create_table(
            "channel_connections",
    ...
    if "channel_oauth_states" not in existing_tables:
        op.create_table(
    ...
    if "threads_meta" not in existing_tables:
        op.create_table(
            "threads_meta",
            sa.Column("thread_id", sa.String(length=64), nullable=False),
            sa.Column("assistant_id", sa.String(length=128), nullable=True),
    ...
    if "channel_conversations" not in existing_tables:
        op.create_table(
