...
def _reflect_state(sync_conn: Any) -> dict[str, bool]:
    ...
    - ``has_kkoclaw_tables``: True iff at least one table that ``Base.metadata``
    ...
    try:
        import kkoclaw.persistence.models  # noqa: F401
    except ImportError:
        logger.debug("kkoclaw.persistence.models not found; metadata may be incomplete")
    ...
    return {
        ...
        "has_kkoclaw_tables": bool(reflected & metadata_tables),
    }
...
def _decide_state(state: dict[str, bool]) -> str:
    ...
    if state["has_alembic_version"]:
        return "versioned"
    if not state["has_kkoclaw_tables"]:
...
def _run_create_all_sync(sync_conn: Any) -> None:
    ...
    try:
        ...
        logger.debug("kkoclaw.persistence.models not found; bootstrap will create empty schema")
...
def _run_baseline_create_all_sync(sync_conn: Any) -> None:
    ...
    try:
        ...
        logger.debug("kkoclaw.persistence.models not found; baseline backfill may be incomplete")
