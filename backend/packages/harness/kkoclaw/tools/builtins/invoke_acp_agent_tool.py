...
def _build_mcp_servers() -> dict[str, dict[str, Any]]:
    """Build ACP ``mcpServers`` config from KKOCLAW's enabled MCP servers."""
...
def _build_acp_mcp_servers() -> list[dict[str, Any]]:
    ...
    The ACP client expects a list of server objects, while KKOCLAW's MCP helper
...
def build_invoke_acp_agent_tool(agents: dict) -> BaseTool:
    ...
    description = (
        ...
        "Do NOT include your workspace paths in the prompt. "
        ...
        try:
            ...
            async with spawn_agent_process(client, cmd, *args, env=agent_env, cwd=physical_cwd) as (conn, proc):
                ...
                await conn.initialize(
                    ...
                    client_info=Implementation(name="kkoclaw", title="KKOCLAW", version="0.1.0"),
                )
