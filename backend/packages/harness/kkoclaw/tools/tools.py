...
def _is_host_bash_tool(tool: object) -> bool:
    ...
    if group == "bash":
        return True
    if use == "kkoclaw.sandbox.tools:bash_tool":
        return True
    return False
...
def get_available_tools(
    ...
    *,
    ...
    `initialize_mcp_tools()` from kkoclaw.mcp module.
    ...
    if getattr(skill_evolution_config, "enabled", False):
        from kkoclaw.tools.skill_manage_tool import skill_manage_tool
    ...
    #
    # Per-user isolation: authenticated users get MCP tools resolved from
    # the ``user_mcp_servers`` DB table (system-default seeds + their own
    # overrides).  Unauthenticated paths (CLI, tests, DEFAULT_USER_ID)
    # continue to read the global ``extensions_config.json`` directly so
    ...
    if include_mcp:
        try:
            ...
            from kkoclaw.mcp.cache import get_cached_mcp_tools, get_cached_mcp_tools_for_user
            from kkoclaw.runtime.user_context import DEFAULT_USER_ID, get_effective_user_id

            user_id = get_effective_user_id()
            if user_id == DEFAULT_USER_ID:
                # Legacy / unauthenticated path: global config.
                extensions_config = ExtensionsConfig.from_file()
                if extensions_config.get_enabled_mcp_servers():
                    mcp_tools = get_cached_mcp_tools()
            else:
                # Authenticated user: per-user config from DB.
                from kkoclaw.config.user_mcp_config import resolve_user_mcp_config_sync

                extensions_config = resolve_user_mcp_config_sync(user_id)
                if extensions_config.get_enabled_mcp_servers():
                    mcp_tools = get_cached_mcp_tools_for_user(
                        user_id, extensions_config=extensions_config
                    )

            if mcp_tools:
                logger.info(f"Using {len(mcp_tools)} cached MCP tool(s) for user '{user_id}'")

                # When tool_search is enabled, register MCP tools in the
                # deferred registry and add tool_search to builtin tools.
                if config.tool_search.enabled:
                    from kkoclaw.tools.builtins.tool_search import DeferredToolRegistry, set_deferred_registry
                    from kkoclaw.tools.builtins.tool_search import tool_search as tool_search_tool
            ...
            logger.error(f"Failed to get cached MCP tools: {e}", exc_info=True)

    # Add invoke_acp_agent tool if any ACP agents are configured
    acp_tools: list[BaseTool] = []
    try:
        from kkoclaw.tools.builtins.invoke_acp_agent_tool import build_invoke_acp_agent_tool
