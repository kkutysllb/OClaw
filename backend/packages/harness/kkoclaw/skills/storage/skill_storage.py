...
from kkoclaw.skills.types import SKILL_MD_FILE, Skill, SkillCategory  # noqa: F401
...
def _infer_mode_scope(
    category: SkillCategory,
    category_root: Path,
    relative_path: Path,
) -> str | None:
    """Infer the work-mode binding tag from the skill's physical location.

    Only the modern ``builtin/`` layout carries mode sub-directories
    (``core`` / ``task`` / ``coding``). Skills under the legacy ``public/``
    layout or the ``custom/`` directory have no automatic mode binding and
    return ``None``.

    Args:
        category: The skill's source category.
        category_root: The category root directory (e.g. ``.../skills/builtin``).
        relative_path: Path from the category root to the skill directory.

    Returns:
        The first path component (e.g. "core", "task", "coding") for builtin
        skills, or ``None`` for custom / legacy skills.
    """
    if category != SkillCategory.PUBLIC:
        return None
    # The modern layout uses the ``builtin`` directory name; the legacy layout
    # used ``public``. Only the modern layout has mode sub-directories.
    if category_root.name != SkillCategory.PUBLIC.value:
        return None
...
class SkillStorage(ABC):
    ...
    def get_skills_root_path(self) -> Path:
        ...
        Origin: ``kkoclaw.skills.loader.get_skills_root_path``.
        """
    ...
    def _iter_skill_files(self) -> Iterable[tuple[SkillCategory, Path, Path]]:
        ...
        ``kkoclaw.skills.loader.load_skills``.
        """
    ...
    def read_custom_skill(self, name: str) -> str:
        ...
        Origin: ``kkoclaw.skills.manager.read_custom_skill_content``.
        """
    ...
    def write_custom_skill(self, name: str, relative_path: str, content: str) -> None:
        ...
        Origin: ``kkoclaw.skills.manager.atomic_write``.
        """
    ...
    def custom_skill_exists(self, name: str) -> bool:
        """Origin: ``kkoclaw.skills.manager.custom_skill_exists``."""

    @abstractmethod
    def public_skill_exists(self, name: str) -> bool:
        """Origin: ``kkoclaw.skills.manager.public_skill_exists``."""

    @abstractmethod
    def append_history(self, name: str, record: dict) -> None:
        """Append a JSONL history entry for ``name``.

        Origin: ``kkoclaw.skills.manager.append_history``.
        """
    ...
    def read_history(self, name: str) -> list[dict]:
        ...
        Origin: ``kkoclaw.skills.manager.read_history``.
        """
    ...
    def get_container_root(self) -> str:
        """Origin: ``kkoclaw.config.skills_config.SkillsConfig.container_path`` accessor."""
        return self._container_root

    def get_custom_skill_dir(self, name: str) -> Path:
        """Path to ``custom/<name>``. Does not create the directory.

        Origin: ``kkoclaw.skills.manager.get_custom_skill_dir``.
        """
    ...
    def get_custom_skill_file(self, name: str) -> Path:
        ...
        Origin: ``kkoclaw.skills.manager.get_custom_skill_file``.
        """
        normalized_name = self.validate_skill_name(name)
    ...
    def get_skill_history_file(self, name: str) -> Path:
        ...
        Origin: ``kkoclaw.skills.manager.get_skill_history_file``.
        """
        normalized_name = self.validate_skill_name(name)
    ...
    def load_skills(self, *, enabled_only: bool = False) -> list[Skill]:
        ...
        Origin: ``kkoclaw.skills.loader.load_skills``.
        """
        from kkoclaw.skills.parser import parse_skill_file
    ...
    def ensure_custom_skill_is_editable(self, name: str) -> None:
        """Origin: ``kkoclaw.skills.manager.ensure_custom_skill_is_editable``."""
