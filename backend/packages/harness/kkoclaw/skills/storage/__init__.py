...
Mirrors the pattern used by ``kkoclaw/sandbox/sandbox_provider.py``.

The process-wide singleton (:func:`get_or_new_skill_storage`) is used for
reading **public** skills (global, read-only). For user-scoped custom skill
operations, use :func:`get_or_new_user_skill_storage` instead — it returns a
per-user :class:`UserScopedSkillStorage` cached with true LRU eviction, ported
from the upstream deer-flow skills subsystem in the engine re-sync.
"""
...
from kkoclaw.skills.storage.user_scoped_skill_storage import UserScopedSkillStorage
...
def get_or_new_skill_storage(**kwargs) -> SkillStorage:
    ...
    **Singleton** is returned (created on first call, then reused) when neither
    ...
    # the global out from under a concurrent read. Construct *inside* the lock —
    # SkillStorage has no teardown hook, so an orphaned instance from a losing
    # racer could not be cleaned up.
...
def get_or_new_user_skill_storage(user_id: str, **kwargs) -> SkillStorage:
    ...
    """
    from kkoclaw.config.paths import make_safe_user_id
    ...
    with _user_scoped_storage_lock:
        ...
        while len(_user_scoped_storages) > _MAX_USER_SCOPED_STORAGES:
            ...
            logger.info(
                "Evicted user-scoped skill storage for safe_id=%s (cache ceiling %d)",
                evicted_key,
                _MAX_USER_SCOPED_STORAGES,
            )
        return cached
