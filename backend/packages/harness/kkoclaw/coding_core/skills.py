...
class CodingSkillRegistry:
    ...
    def discover(cls, project_root: str | None = None) -> list[CodingSkill]:
        ...
        if builtin_root is not None:
            ...
            # The unified builtin layout (commit 4548d88) nests the
            # coding skill-pack under builtin/coding/coding/.  Scan it
            # as an additional root so those 59 skills are discovered.
            skill_pack = builtin_root / "coding"
            if skill_pack.is_dir():
                roots.append((skill_pack, "global"))
...
def _builtin_coding_skills_root() -> Path | None:
    try:
        ...
        root = get_app_config().skills.get_skills_path() / "builtin" / "coding"
    except Exception:
        return None
