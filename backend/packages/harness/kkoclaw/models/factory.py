...
def _enable_stream_usage_by_default(model_use_path: str, model_settings_from_config: dict) -> None:
    ...
    base URL or client is configured. KKOCLAW frequently uses OpenAI-compatible
...
def _apply_stream_chunk_timeout_default(model_class: type, model_settings_from_config: dict) -> None:
    ...
    it is accepted by ``ChatOpenAI`` and by every KKOCLAW provider that subclasses
    ...
    explicit override) automatically.
...
def create_chat_model(name: str | None = None, thinking_enabled: bool = False, *, app_config: AppConfig | None = None, attach_tracing: bool = True, **kwargs) -> BaseChatModel:
    ...
    # KKOCLAW: enable stream_usage for OpenAI-compatible gateways (string-path based,
    # so it also covers the common ``langchain_openai:ChatOpenAI`` + custom base_url case).
    _enable_stream_usage_by_default(model_config.use, model_settings_from_config)

    # For Codex Responses API models: map thinking mode to reasoning_effort
    from kkoclaw.models.openai_codex_provider import CodexChatModel
    ...
    if max_input_tokens is not None and getattr(model_instance, "profile", None) is None:
        try:
            model_instance.profile = {"max_input_tokens": max_input_tokens}
            logger.debug("Injected model profile for '%s': max_input_tokens=%d", name, max_input_tokens)
        except (AttributeError, TypeError):
            logger.debug("Could not set profile on model '%s'", name)
