...
def _migrate_legacy_project_home() -> None:
    """Move ``<project_root>/.kkoclaw`` to ``~/.kkoclaw`` if needed.

    The web deployment previously defaulted ``KKOCLAW_HOME`` to a project-local
    directory (``backend/.kkoclaw`` when the gateway is launched from the
    monorepo). Storing user data inside the repo is undesirable — it pollutes
    ``git status`` and forces per-machine ``.gitignore`` entries — so the
    default was moved to the user's home directory. To avoid losing any
    existing data, we run a one-shot migration the first time the new code
    path is reached.

    Migration rules (idempotent, runs at most once per process):

    * If ``KKOCLAW_HOME`` is explicitly set, do nothing — the operator has
      chosen a location and we respect that.
    * If the new target ``runtime_home()`` already exists, do nothing —
      previous run already migrated, or the user started from scratch.
    * If a legacy ``<project_root>/.kkoclaw`` directory exists, move it to
      the target via ``shutil.move`` and emit a single ``INFO`` log line.
    * If both exist, log a loud ``WARNING`` and leave both alone — the
      operator must reconcile manually. We never silently overwrite data.
    * If neither exists, do nothing — the next write to ``base_dir`` will
      create the new directory via the standard mkdir path.
    """
    # Respect explicit user override: they have already chosen a location.
    if os.getenv("KKOCLAW_HOME"):
        return

    target = runtime_home()

    # Already migrated (or fresh start): nothing to do.
    if target.exists():
        return

    legacy = project_root() / ".kkoclaw"
    if not legacy.is_dir():
        return

    # Both exist: refuse to clobber. Operator must reconcile.
    # (Currently unreachable because we just checked target.exists() above,
    # but kept for defensive clarity in case the contract changes.)
    if target.exists() and legacy.is_dir():
        logger.warning(
            "Both legacy KKOCLAW_HOME %s and new home %s exist; leaving both in place. "
            "Move the legacy directory aside before continuing.",
            legacy,
            target,
        )
        return
    ...
    try:
        ...
        logger.info(
            "Migrated legacy KKOCLAW_HOME: %s -> %s. "
            "Future runs will use the home-directory location automatically.",
            legacy,
            target,
        )
    except Exception as exc:  # noqa: BLE001 — migration is best-effort
        logger.warning(
            "Failed to migrate legacy KKOCLAW_HOME from %s to %s: %s. "
            "The gateway will start with an empty %s and the legacy data "
...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable KKOCLAW state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.kkoclaw``).  Using
...
class Paths:
    """
    Centralized path configuration for KKOCLAW application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. `KKOCLAW_HOME` environment variable
        3. User home fallback: `~/.kkoclaw` (Unix dotdir convention, mirrors
           the desktop layout under `~/.kkoclaw-desktop`).

    On first construction the class also runs :func:`_migrate_legacy_project_home`
    which moves any pre-existing `<project_root>/.kkoclaw` data into
    `~/.kkoclaw` so users upgrading from older web deployments do not lose
    their threads, agents, or memory.
    """

    def __init__(self, base_dir: str | Path | None = None) -> None:
        # One-shot migration: legacy project-local KKOCLAW_HOME → ~/.kkoclaw.
        # Skipped when the caller supplied an explicit base_dir (typically
        # tests, where the test harness already controls the directory).
        if base_dir is None:
            _migrate_legacy_project_home()
    ...
    def host_base_dir(self) -> Path:
        ...
        Set KKOCLAW_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("KKOCLAW_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("KKOCLAW_HOME"):
