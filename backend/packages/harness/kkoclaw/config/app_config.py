...
def apply_logging_level(name: str | None) -> None:
    """Resolve *name* to a logging level and apply it to the ``kkoclaw``/``app`` logger hierarchies.

    Only the ``kkoclaw`` and ``app`` logger levels are changed so that
    ...
    for logger_name in ("kkoclaw", "app"):
...
class AppConfig(BaseModel):
    ...
    cron_management: CronManagementConfig = Field(default_factory=CronManagementConfig, description="Agent-managed cron job configuration")
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `KKOCLAW_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("KKOCLAW_CONFIG_PATH"):
            path = Path(os.getenv("KKOCLAW_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `KKOCLAW_CONFIG_PATH` not found at {path}")
            return path
        else:
...
_current_app_config: ContextVar[AppConfig | None] = ContextVar("kkoclaw_current_app_config", default=None)
_current_app_config_stack: ContextVar[tuple[AppConfig | None, ...]] = ContextVar("kkoclaw_current_app_config_stack", default=())
...
def get_app_config() -> AppConfig:
    """Get the KKOCLAW config instance.
