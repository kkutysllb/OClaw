...
class AgentConfig(BaseModel):
    ...
    # work_mode_id associates this agent with a work-mode preset (task/coding).
    # - None (or omitted): no mode association — the caller's runtime work_mode_id
    #   (if any) controls skill availability. This preserves backward compatibility.
    # - "task" / "coding": the agent declares which mode it belongs to, so the
    #   runtime can resolve the effective skill set from the mode's config.
