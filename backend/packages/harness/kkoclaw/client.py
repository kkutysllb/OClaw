"""KKOCLAWClient — Embedded Python client for KKOCLAW agent system.

Provides direct programmatic access to KKOCLAW's agent capabilities
...
from kkoclaw.uploads.manager import (
...
class KKOCLAWClient:
    """Embedded Python client for KKOCLAW agent system.
    ...
    def _ensure_agent(self, config: RunnableConfig):
        ...
        if model_name and self._app_config:
            mc = self._app_config.get_model_config(model_name)
            if mc and mc.display_name:
                model_display_name = mc.display_name
    ...
    def _ai_tool_calls_event(msg_id: str | None, tool_calls) -> "StreamEvent":
        ...
        return StreamEvent(
            ...
            data={
                ...
                "content": "",
                "id": msg_id,
                "tool_calls": KKOCLAWClient._serialize_tool_calls(tool_calls),
            },
        )
    ...
    def _tool_message_event(msg: ToolMessage) -> "StreamEvent":
        ...
        return StreamEvent(
            ...
            data={
                "type": "tool",
                "content": KKOCLAWClient._extract_text(msg.content),
                "name": msg.name,
    ...
    def _serialize_message(msg) -> dict:
        ...
        if isinstance(msg, AIMessage):
            ...
            if msg.tool_calls:
                d["tool_calls"] = KKOCLAWClient._serialize_tool_calls(msg.tool_calls)
    ...
    def list_threads(self, limit: int = 10) -> dict:
        ...
        checkpointer = self._checkpointer
        if checkpointer is None:
            from kkoclaw.runtime.checkpointer.provider import get_checkpointer
    ...
    def stream(
        ...
        * ``StreamBridge`` is an asyncio-queue decoupling producers from
        ...
        So ``KKOCLAWClient.stream()`` is a parallel, sync, in-process
    ...
    def import_memory(self, memory_data: dict) -> dict:
        ...
        from kkoclaw.agents.memory.updater import import_memory_data
    ...
    def update_mcp_config(self, mcp_servers: dict[str, dict]) -> dict:
        ...
        if config_path is None:
            raise FileNotFoundError("Cannot locate extensions_config.json. Set KKOCLAW_EXTENSIONS_CONFIG_PATH or ensure it exists in the project root.")
    ...
    def reload_memory(self) -> dict:
        ...
        """
        from kkoclaw.agents.memory.updater import reload_memory_data

        return reload_memory_data(user_id=get_effective_user_id())

    def clear_memory(self) -> dict:
        """Clear all persisted memory data."""
        from kkoclaw.agents.memory.updater import clear_memory_data
    ...
    def create_memory_fact(self, content: str, category: str = "context", confidence: float = 0.5) -> dict:
        ...
        from kkoclaw.agents.memory.updater import create_memory_fact

        return create_memory_fact(content=content, category=category, confidence=confidence)

    def delete_memory_fact(self, fact_id: str) -> dict:
        """Delete a single fact from memory by fact id."""
        from kkoclaw.agents.memory.updater import delete_memory_fact
    ...
    def update_memory_fact(
        ...
        from kkoclaw.agents.memory.updater import update_memory_fact
    ...
    def delete_upload(self, thread_id: str, filename: str) -> dict:
        ...
        """
        from kkoclaw.utils.file_conversion import CONVERTIBLE_EXTENSIONS
