...
class CodingSession:
    ...
    def _build_agent(self):
        """Build the agent graph via the unified Lead Agent factory.

        The coding work mode is selected via ``work_mode_id="coding"`` in
        the runtime config — the Lead Agent's coding branch then loads the
        coding tool set, qiongqi prompt, and CodingThreadState.
        """
    ...
    def _make_config(self) -> RunnableConfig:
        ...
        configurable = {
            ...
            # Select the Lead Agent's coding branch so the coding tool set,
            # qiongqi prompt, and CodingThreadState are activated.
            "work_mode_id": "coding",
        }
        if self.model_name:
