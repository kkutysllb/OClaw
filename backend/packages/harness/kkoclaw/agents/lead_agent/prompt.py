...
def _invalidate_enabled_skills_cache() -> threading.Event:
    ...
    with _enabled_skills_lock:
        ...
        # Also invalidate the builtin-by-scope cache so directory-based
        # work-mode binding picks up newly installed/removed/moved skills.
        # Lazy import avoids a circular dependency at module load time.
        from kkoclaw.skills.work_modes import invalidate_builtin_skills_cache
...
{soul}
{work_mode_context}{self_update_section}
...
KKOCLAW is an open-source AI agent framework that gained significant traction in early 2026
[citation:GitHub Repository](https://github.com/kkoclaw/kkoclaw). The project focuses on
providing a production-ready agent system with sandbox execution and memory management
[citation:KKOCLAW Documentation](https://kkoclaw.dev/docs).
...
### Primary Sources
- [GitHub Repository](https://github.com/kkoclaw/kkoclaw) - Official source code and documentation
- [KKOCLAW Documentation](https://kkoclaw.dev/docs) - Technical specifications
...
- ✅ RIGHT in Sources: `[GitHub Repository](https://github.com/kkoclaw/kkoclaw) - 官方源代码和文档`
...
def get_skills_prompt_section(
    ...
    *,
    ...
    When ``work_mode_id`` is provided, the effective skill set is resolved
    via ``compute_effective_skills`` which intersects the agent's whitelist
    with the mode's preset and unconditionally preserves locked core skills.
    When ``work_mode_id`` is None, the legacy behavior is preserved (the
    caller-supplied ``available_skills`` is used directly, or no filtering
    if it's None).
    """
    # If a work mode is active, fold it through the canonical resolution
    # function so the same effective-skill-set contract applies here as
    # everywhere else. ``compute_effective_skills`` returns None when both
    # the work mode and the agent whitelist are absent, signaling "no
    ...
    if work_mode_id is not None:
        ...
        resolved = compute_effective_skills(
            agent_skills_input,
            work_mode_id,
            extensions_config,
...
def _build_work_mode_context(
    ...
    """Generate the ``<work_mode_context>`` block for the system prompt.

    The block makes the lead agent explicitly aware of which work mode it
    is operating under, including:
    - Mode display name and purpose
    - Bound lead agent identity
    - Effective skill count and sample names
    - Focus areas
    - Mode-specific orchestration guidance

    Returns an empty string when ``work_mode_id`` is None/empty or unknown,
    preserving backward compatibility for callers that haven't opted into
    the work-mode contract.
    """
    if not work_mode_id:
        return ""
    try:
        ...
        return ""
    mode_cfg = cfg.work_modes.modes.get(work_mode_id)
    if mode_cfg is None:
        return ""

    # Compute the real effective skill set for this mode using the same
    # resolution logic as ``get_skills_prompt_section`` below, so the count
    # reported here matches what the agent actually sees in its
    # ``<skill_system>`` block. The caller-supplied ``available_skills`` is
    # the per-agent whitelist (None for the default lead agent) — it is
    # folded through ``compute_effective_skills`` together with the mode's
    # directory-binding + locked-skill rules to get the truthful set.
    #
    # Without this, the context block reported "0 skills" for the default
    # lead agent (whose available_skills is None) even though the skill
    # list section correctly showed the mode's bound skills.
    try:
        ...
        # always a concrete set. Fall back defensively anyway.
        skill_set: set[str] = effective if effective is not None else (available_skills or set())
    except Exception:
        logger.exception("Failed to compute effective skills for work-mode context")
        skill_set = set(available_skills or ())
    skill_count = len(skill_set)
    sample_skills = sorted(skill_set)[:5]
    sample_str = ", ".join(sample_skills) if sample_skills else "N/A"
    focus_str = ", ".join(mode_cfg.focus_areas) if mode_cfg.focus_areas else "general"
    lead_name = mode_cfg.lead_agent_name or mode_cfg.name
    purpose = mode_cfg.description or "General-purpose"
    hint = mode_cfg.orchestration_hint or (
        "No mode-specific guidance — use your default judgment."
    )
    return f"""<work_mode_context>
You are currently operating in **{mode_cfg.name}** mode (`{work_mode_id}`).

**Mode purpose**: {purpose}

**Lead Agent**: {lead_name}

**Bound skills**: {skill_count} skill(s) loaded in this mode (e.g. {sample_str}...).
...
**Reminder**: Your skill set, tool priorities, and response style should align with this mode. If the user's request clearly falls outside this mode's focus areas, you may still proceed but should note the mismatch.
</work_mode_context>
"""
...
def apply_prompt_template(
    ...
    *,
    ...
    # Get skills section
    skills_section = get_skills_prompt_section(available_skills, app_config=app_config, work_mode_id=work_mode_id)
    ...
    prompt = SYSTEM_PROMPT_TEMPLATE.format(
        ...
        work_mode_context=_build_work_mode_context(work_mode_id, None, available_skills),
