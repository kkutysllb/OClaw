...
def _create_summarization_middleware(*, app_config: AppConfig | None = None) -> KKOCLAWSummarizationMiddleware | None:
    ...
    # the sole entry point for KKOCLAWSummarizationMiddleware, and the runtime
    # config is not expected to change after startup.
    skills_container_path = resolved_app_config.skills.container_path or "/mnt/skills"

    return KKOCLAWSummarizationMiddleware(
        **kwargs,
...
def _create_todo_list_middleware(is_plan_mode: bool) -> TodoMiddleware | None:
    ...
    # Custom prompts matching KKOCLAW's style
    system_prompt = """
<todo_list_system>
...
def _build_middlewares(
    ...
    *,
    ...
    if resolved_app_config.tool_search.enabled:
        from kkoclaw.agents.middlewares.deferred_tool_filter_middleware import DeferredToolFilterMiddleware
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    from kkoclaw.tools.builtins import setup_agent
    ...
    # Work mode (task/coding) — read from runtime config, falling back to the
    ...
    logger.info(
        "Create Agent(%s) -> thinking_enabled: %s, reasoning_effort: %s, model_name: %s, is_plan_mode: %s, subagent_enabled: %s, max_concurrent_subagents: %s, work_mode_id: %s",
    ...
    # ── Coding mode branch ────────────────────────────────────────────
    # When the Lead Agent runs under the coding work mode it swaps in the
    # coding-specialised tool set, the qiongqi-engine prompt, the coding
    # middleware chain (skills injection + tool policy + post-edit verify
    # + ROI telemetry), and the extended CodingThreadState schema. This
    # replaces the retired standalone ``coding_agent`` graph — the same
    # capabilities, now served by the unified Lead Agent factory.
    if work_mode_id == "coding":
        from kkoclaw.agents.lead_agent.coding_mode import (
            build_coding_mode_middlewares,
            build_coding_system_prompt_for_metadata,
            get_coding_mode_tools,
        )
        from kkoclaw.agents.thread_state import CodingThreadState
        ...
        tools = get_coding_mode_tools(
            app_config=resolved_app_config,
            model_name=model_name,
            subagent_enabled=subagent_enabled,
        )

        stable_prompt = build_coding_system_prompt_for_metadata(
            qiongqi_engine=qiongqi_engine,
            model_display_name=model_display_name,
            is_plan_mode=is_plan_mode,
            subagent_enabled=subagent_enabled,
            max_concurrent_subagents=max_concurrent_subagents,
        ...
        coding_middlewares = build_coding_mode_middlewares(
            qiongqi_engine=qiongqi_engine,
            app_config=resolved_app_config,
        ...
        logger.info(
            "Lead Agent entering coding mode -> project_root: %s, thread_id: %s, tools: %d",
