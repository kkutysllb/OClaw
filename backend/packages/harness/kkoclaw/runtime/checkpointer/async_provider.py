...
For sync usage see :mod:`kkoclaw.runtime.checkpointer.provider`.
"""
...
from kkoclaw.runtime.checkpointer.provider import (
    POSTGRES_CONN_REQUIRED,
    POSTGRES_INSTALL,
    SQLITE_INSTALL,
)
from kkoclaw.runtime.store._sqlite_utils import ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
async def _async_checkpointer_from_database(db_config) -> AsyncIterator[Checkpointer]:
    ...
    if db_config.backend == "sqlite":
        ...
        # Route the relative path through resolve_sqlite_conn_str so the
        # connection string is anchored to the runtime home (``~/.kkoclaw``)
        # instead of the CWD — otherwise launching ``cd backend && uvicorn``
        # dropped a hidden ``backend/.kkoclaw/checkpoints.db`` into the repo.
        conn_str = resolve_sqlite_conn_str(db_config.checkpointer_sqlite_path)
