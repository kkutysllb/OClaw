...
``{base_dir}/threads/{thread_id}/runs/{run_id}.jsonl`` where ``base_dir``
defaults to the runtime home (``~/.kkoclaw`` on web, ``~/.kkoclaw-desktop``
on desktop) — see :func:`kkoclaw.config.runtime_paths.runtime_home`.
...
class JsonlRunEventStore(RunEventStore):
    def __init__(self, base_dir: str | Path | None = None):
        # Fall back to runtime_home() (defaults to ~/.kkoclaw) instead of
        # the legacy CWD-relative ".kkoclaw" — the latter used to drop a
        # hidden directory into the working tree (typically backend/.kkoclaw)
        # on every dev launch, polluting the repo and tripping git status.
        self._base_dir = Path(base_dir) if base_dir else runtime_home()
