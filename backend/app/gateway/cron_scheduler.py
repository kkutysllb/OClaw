...
class CronScheduler:
    ...
    async def _invoke_via_client(
        ...
        *workspace_thread_id* and *workspace_user_id* are passed via
        ...
        # Use the same 500-step ceiling as web/channel runs so cron-driven
        # agent tasks don't terminate prematurely on long workflows.
        config: dict[str, Any] = {"recursion_limit": 500}
        if model:
