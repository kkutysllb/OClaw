...
from kkoclaw.runtime.runs.naming import resolve_root_run_name
...
_CONTEXT_CONFIGURABLE_KEYS: frozenset[str] = frozenset(
    {
        ...
        "project_id",
        "memory_scope",
        # Work mode preset id (task/coding). Read by skill resolution to
...
def resolve_agent_factory(assistant_id: str | None):
    ...
    Since the standalone ``coding_agent`` graph was retired, every assistant
    id resolves to :func:`make_lead_agent`. Coding mode is now selected by
    the ``work_mode_id`` runtime key (see :func:`build_run_config` which
    auto-injects ``work_mode_id="coding"`` for legacy ``coding_agent`` /
    ``coding-agent`` assistant ids so existing threads keep working).
    """
...
def build_run_config(
    ...
    *,
    ...
    #
    # Legacy ``coding_agent`` / ``coding-agent`` assistant ids no longer map
    # to a dedicated graph — instead they auto-select the coding work mode so
    # old threads (created before the graph merger) keep getting the coding
    # tool set and CodingThreadState from the unified Lead Agent factory.
    normalized_assistant = (assistant_id or "").strip().lower()
    is_legacy_coding_assistant = normalized_assistant in ("coding_agent", "coding-agent")
    if assistant_id and assistant_id != _DEFAULT_ASSISTANT_ID and not is_legacy_coding_assistant:
    ...
    if is_legacy_coding_assistant:
        # Force the coding work mode on so the Lead Agent's coding branch
        # activates, regardless of what the client's context/configurable
        # already carries. This preserves the old coding_agent contract.
...
async def start_run(
    ...
    # Merge KKOCLAW-specific context overrides into both ``configurable`` and ``context``.
