...
:class:`kkoclaw.agents.runs.RunManager` and
:class:`kkoclaw.agents.stream_bridge.StreamBridge`.
...
class RunCreateRequest(BaseModel):
    ...
    context: dict[str, Any] | None = Field(default=None, description="KKOCLAW context overrides (model_name, thinking_enabled, etc.)")
...
async def stream_existing_run(
    ...
    # If the run has already finished, return a minimal "end" SSE stream
    # so that reconnecting clients (useStream reconnectOnMount) get a
    # clean termination instead of a 500 from a stale bridge queue.
    if record.status in (RunStatus.success, RunStatus.error, RunStatus.interrupted):
        async def _ended_stream():
...
async def thread_token_usage(
    ...
    return ThreadTokenUsageResponse(thread_id=thread_id, **resolved)
