...
OClaw 是一个开源的超级智能体运行时（Super Agent Harness），基于 LangGraph 和 LangChain 构建。它将子智能体（Sub-Agents）、长期记忆（Memory）和沙箱执行环境（Sandbox）整合在一起，配合可扩展的技能系统（Skills），使智能体能够完成复杂、多步骤的任务。
...
| 模式 | 提供者 | 说明 |
|------|--------|------|
| 本地执行 | LocalSandboxProvider | 直接在宿主机执行 |
...
| 变量 | 说明 |
|------|------|
| `OClaw_PROJECT_ROOT` | 项目根目录 |
| `OClaw_CONFIG_PATH` | 自定义配置文件路径 |
| `OClaw_HOME` | 运行时数据目录 |
| `OClaw_SKILLS_PATH` | 自定义技能目录 |
...
OClaw 可作为内嵌 Python 库使用，无需启动完整 HTTP 服务：
