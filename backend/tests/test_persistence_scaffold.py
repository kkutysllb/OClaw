...
from kkoclaw.runtime.runs.store.memory import MemoryRunStore
...
class TestDatabaseConfig:
    ...
    def test_sqlite_paths_unified(self):
        ...
        assert c.sqlite_path.endswith("kkoclaw.db")
    ...
    def test_relative_sqlite_dir_anchored_to_runtime_home(self, tmp_path, monkeypatch):
        """Regression: relative ``sqlite_dir`` used to resolve against CWD,
        dropping ``backend/.kkoclaw/data/`` into the working tree on every
        dev launch. It must now anchor to ``runtime_home()`` (``~/.kkoclaw``).
        """
        from kkoclaw.config.database_config import DatabaseConfig as _DB
        ...
        c = _DB(backend="sqlite", sqlite_dir="data")
        assert c.sqlite_path == str((fake_home / ".kkoclaw" / "data" / "kkoclaw.db").resolve())

    def test_absolute_sqlite_dir_honoured_as_is(self, tmp_path):
        """Absolute ``sqlite_dir`` values bypass the runtime-home anchor."""
        from kkoclaw.config.database_config import DatabaseConfig as _DB

        abs_dir = tmp_path / "var" / "lib" / "kkoclaw"
        c = _DB(backend="sqlite", sqlite_dir=str(abs_dir))
        assert c.sqlite_path == str((abs_dir / "kkoclaw.db").resolve())

    def test_empty_sqlite_dir_falls_back_to_runtime_home_data(self, tmp_path, monkeypatch):
        """Empty ``sqlite_dir`` lands at ``<runtime_home>/data/``."""
        from kkoclaw.config.database_config import DatabaseConfig as _DB
        ...
        c = _DB(backend="sqlite", sqlite_dir="")
        assert c.sqlite_path == str((fake_home / ".kkoclaw" / "data" / "kkoclaw.db").resolve())

    def test_init_engine_from_config_uses_resolved_sqlite_dir(self, tmp_path, monkeypatch):
        """Regression: ``init_engine_from_config`` used to forward the raw
        ``config.sqlite_dir`` (e.g. ``.kkoclaw/data``) to ``init_engine``,
        whose ``os.makedirs(sqldir)`` then dropped ``backend/.kkoclaw/data/``
        into the working tree — even though the engine itself wrote the db
        file to ``<runtime_home>/data/`` via ``config.app_sqlalchemy_url``.

        The two paths must agree: the directory ``init_engine`` creates is
        the parent of the URL it opens.
        """
        from kkoclaw.config.database_config import DatabaseConfig as _DB
        from kkoclaw.persistence import engine as engine_module
        from kkoclaw.persistence.engine import init_engine_from_config
        ...
        # Capture the sqlite_dir argument init_engine_from_config passes through.
        seen: dict[str, str] = {}

        async def _fake_init_engine(backend, url="", echo=False, pool_size=5, sqlite_dir=""):  # noqa: ARG001
            seen["sqlite_dir"] = sqlite_dir
            seen["url"] = url

        monkeypatch.setattr(engine_module, "init_engine", _fake_init_engine)

        c = _DB(backend="sqlite", sqlite_dir=".kkoclaw/data")

        import asyncio
        asyncio.run(init_engine_from_config(c))

        # Both the URL and the directory must be anchored to the runtime home.
        # The directory must NOT be CWD-relative (no leading '.kkoclaw/data'
        # segment, no bare '.', and no stray leading './' that would mean
        # ``os.makedirs`` was given the raw config string).
        assert seen["sqlite_dir"] == str((fake_home / ".kkoclaw" / "data").resolve())
        assert "kkoclaw.db" in seen["url"]
        assert seen["url"].startswith("sqlite+aiosqlite:///")
        # Path must start with the fake home absolute path, not a CWD-relative
        # segment like "./data" or ".kkoclaw/data".
        from pathlib import Path
        assert Path(seen["sqlite_dir"]).is_absolute()
        assert seen["sqlite_dir"].startswith(str(fake_home.resolve()))
        # And, critically, the URL must not double-nest under runtime home.
        assert "/.kkoclaw/.kkoclaw/" not in seen["url"]

    def test_app_sqlalchemy_url_sqlite(self):
        c = DatabaseConfig(backend="sqlite", sqlite_dir="./data")
        url = c.app_sqlalchemy_url
        assert url.startswith("sqlite+aiosqlite:///")
        assert "kkoclaw.db" in url
...
class TestEngineLifecycle:
    ...
    async def test_memory_is_noop(self):
        from kkoclaw.persistence.engine import close_engine, get_session_factory, init_engine
    ...
    async def test_postgres_without_asyncpg_gives_actionable_error(self):
        ...
        from kkoclaw.persistence.engine import init_engine
