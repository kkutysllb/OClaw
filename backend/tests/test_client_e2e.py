"""End-to-end tests for KKOCLAWClient.
...
Only KKOCLAW_HOME is redirected to tmp_path for filesystem isolation.
...
from kkoclaw.client import KKOCLAWClient, StreamEvent
...
def _make_e2e_config() -> AppConfig:
    ...
    - ``E2E_MODEL_NAME``  (default: ``generic-model``)
    - ``E2E_MODEL_USE``   (default: ``langchain_openai:ChatOpenAI``)
    - ``E2E_MODEL_ID``    (default: ``ep-20251211175242-llcmh``)
    - ``E2E_BASE_URL``    (default: ``https://ark-cn-beijing.example.com/api/v3``)
    ...
    return AppConfig.model_validate(
        {
            "models": [
                {
                    "name": os.getenv("E2E_MODEL_NAME", "generic-model"),
                    ...
                    "base_url": os.getenv("E2E_BASE_URL", "https://ark-cn-beijing.example.com/api/v3"),
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - KKOCLAW_HOME → tmp_path (all thread data lands in a temp dir)
    - KKOCLAW_PROJECT_ROOT → repository root (shared skills/config assets
    ...
    monkeypatch.setattr("kkoclaw.config.paths._paths", None)
    monkeypatch.setattr("kkoclaw.sandbox.sandbox_provider._default_sandbox_provider", None)
    ...
    monkeypatch.setattr("kkoclaw.config.app_config._app_config", None)
    monkeypatch.setattr("kkoclaw.config.app_config._app_config_is_custom", False)
    config = _make_e2e_config()
    monkeypatch.setattr("kkoclaw.config.app_config._app_config", config)
    monkeypatch.setattr("kkoclaw.config.app_config._app_config_is_custom", True)
    monkeypatch.setattr("kkoclaw.client.get_app_config", lambda: config)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from kkoclaw.config.title_config import TitleConfig

    monkeypatch.setattr("kkoclaw.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from kkoclaw.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "kkoclaw.agents.middlewares.memory_middleware.get_memory_config",
        lambda: MemoryConfig(enabled=False),
    )

    # 5. Ensure summarization is off (default, but be explicit)
    from kkoclaw.config.summarization_config import SummarizationConfig

    monkeypatch.setattr("kkoclaw.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from kkoclaw.agents.lead_agent.agent import _build_middlewares as _original_build_middlewares
    ...
    monkeypatch.setattr("kkoclaw.client._build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A KKOCLAWClient wired to the isolated e2e_env."""
    return KKOCLAWClient(checkpointer=None, thinking_enabled=False)
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = KKOCLAWClient(checkpointer=None, plan_mode=False)
        ...
        c2 = KKOCLAWClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kkoclaw.skills.storage._default_skill_storage",
...
class TestConfigManagement:
    ...
    def test_list_models_returns_injected_config(self, e2e_env):
        ...
        expected_model_name = os.getenv("E2E_MODEL_NAME", "generic-model")
        c = KKOCLAWClient(checkpointer=None, thinking_enabled=False)
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KKOCLAW_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from kkoclaw.config.extensions_config import reload_extensions_config
