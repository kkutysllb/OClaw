...
from kkoclaw.config.acp_config import ACPAgentConfig
from kkoclaw.config.extensions_config import ExtensionsConfig, McpServerConfig, set_extensions_config
from kkoclaw.tools.builtins.invoke_acp_agent_tool import (
...
from kkoclaw.tools.tools import get_available_tools
...
def test_build_mcp_servers_filters_disabled_and_maps_transports():
    ...
    monkeypatch.setattr(
        "kkoclaw.config.extensions_config.ExtensionsConfig.from_file",
...
async def test_build_invoke_tool_description_and_unknown_agent_error():
    ...
    assert "Do NOT include your workspace paths" in tool.description
...
def test_get_work_dir_uses_base_dir_when_no_thread_id(monkeypatch, tmp_path):
    ...
    from kkoclaw.config import paths as paths_module
...
def test_get_work_dir_uses_per_thread_path_when_thread_id_given(monkeypatch, tmp_path):
    ...
    from kkoclaw.config import paths as paths_module
    from kkoclaw.runtime import user_context as uc_module
...
async def test_invoke_acp_agent_skips_invalid_mcp_servers(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setattr(paths_module, "get_paths", lambda: paths_module.Paths(base_dir=tmp_path))
    monkeypatch.setattr(
        "kkoclaw.tools.builtins.invoke_acp_agent_tool._build_acp_mcp_servers",
...
def test_get_available_tools_includes_invoke_acp_agent_when_agents_configured(monkeypatch):
    from kkoclaw.config.acp_config import load_acp_config_from_dict
    ...
    monkeypatch.setattr("kkoclaw.tools.tools.get_app_config", lambda: fake_config)
    monkeypatch.setattr(
        "kkoclaw.config.extensions_config.ExtensionsConfig.from_file",
        classmethod(lambda cls: ExtensionsConfig(mcp_servers={}, skills={})),
    )
...
def test_get_available_tools_uses_explicit_app_config_for_acp_agents(monkeypatch):
    ...
    monkeypatch.setattr("kkoclaw.tools.tools.is_host_bash_allowed", lambda config=None: True)
    monkeypatch.setattr("kkoclaw.config.acp_config.get_acp_agents", fail_get_acp_agents)
    monkeypatch.setattr("kkoclaw.tools.builtins.invoke_acp_agent_tool.build_invoke_acp_agent_tool", fake_build_invoke_acp_agent_tool)
