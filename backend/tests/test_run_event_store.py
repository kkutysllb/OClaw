...
from kkoclaw.runtime.events.store.memory import MemoryRunEventStore
...
class TestDbRunEventStore:
    ...
    async def test_basic_crud(self, tmp_path):
        ...
        from kkoclaw.runtime.events.store.db import DbRunEventStore
...
class TestMakeRunEventStore:
    ...
    async def test_memory_backend_default(self):
        from kkoclaw.runtime.events.store import make_run_event_store
...
class TestJsonlRunEventStore:
    @pytest.mark.anyio
    async def test_basic_crud(self, tmp_path):
        from kkoclaw.runtime.events.store.jsonl import JsonlRunEventStore
    ...
    def test_default_base_dir_uses_runtime_home(self, tmp_path, monkeypatch):
        """Regression: ``JsonlRunEventStore()`` with no ``base_dir`` used to
        fall back to the CWD-relative ``.kkoclaw`` (which, when launched from
        ``backend/``, dropped a hidden directory into the working tree on
        every dev run). It must now follow ``runtime_home()`` → ``~/.kkoclaw``.
        """
        from kkoclaw.runtime.events.store.jsonl import JsonlRunEventStore
        ...
        # No KKOCLAW_HOME set in env → pure home-dir fallback.
        monkeypatch.delenv("KKOCLAW_HOME", raising=False)

        s = JsonlRunEventStore()
        assert s._base_dir == fake_home / ".kkoclaw"
