...
# Make 'app' and 'kkoclaw' importable from any working directory
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "scripts"))

# Break the circular import chain that exists in production code:
#   kkoclaw.subagents.__init__
#     -> .executor (SubagentExecutor, SubagentResult)
#       -> kkoclaw.agents.thread_state
#         -> kkoclaw.agents.__init__
#           -> lead_agent.agent
#             -> subagent_limit_middleware
#               -> kkoclaw.subagents.executor  <-- circular!
#
# By injecting a mock for kkoclaw.subagents.executor *before* any test module
...
sys.modules["kkoclaw.subagents.executor"] = _executor_mock
...
# (see ``kkoclaw.runtime.user_context``). Without this fixture, every
...
def _reset_skill_storage_singleton():
    ...
    try:
        from kkoclaw.skills.storage import reset_skill_storage
    except ImportError:
        yield
        return
    reset_skill_storage()
...
def _auto_user_context(request):
    ...
    try:
        from kkoclaw.runtime.user_context import (
            reset_current_user,
            set_current_user,
        )
    except ImportError:
