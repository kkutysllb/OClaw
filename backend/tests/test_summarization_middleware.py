...
from kkoclaw.agents.middlewares.summarization_middleware import KKOCLAWSummarizationMiddleware, SummarizationEvent
...
) -> KKOCLAWSummarizationMiddleware:
    model = MagicMock()
...
def test_memory_flush_hook_skips_when_memory_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    queue = MagicMock()
    monkeypatch.setattr("kkoclaw.agents.memory.summarization_hook.get_memory_config", lambda: MemoryConfig(enabled=False))
