...
class TestChannelManager:
    def test_get_client_includes_csrf_header_and_cookie(self):
        ...
        manager = ChannelManager(bus=bus, store=store, langgraph_url="http://localhost:9193")
        ...
        assert kwargs["url"] == "http://localhost:9193"
        headers = kwargs["headers"]
        csrf_token = headers["X-CSRF-Token"]
        assert csrf_token
        assert headers["Cookie"] == f"csrf_token={csrf_token}"
        assert headers["X-KKOCLAW-Internal-Token"]
    ...
    def test_handle_feishu_stream_conflict_sends_busy_message(self, monkeypatch):
        ...
        async def go():
            ...
            async def _conflict_stream():
                request = httpx.Request("POST", "http://127.0.0.1:9193/runs")
    ...
    def test_each_topic_creates_new_thread(self):
        """Messages with distinct topic_ids should each create a new KKOCLAW thread."""
        from app.channels.manager import ChannelManager
    ...
    def test_same_topic_reuses_thread(self):
        """Messages with the same topic_id should reuse the same KKOCLAW thread."""
        from app.channels.manager import ChannelManager
...
class TestChannelService:
    ...
    def test_service_urls_fall_back_to_env(self, monkeypatch):
        ...
        monkeypatch.setenv("KKOCLAW_CHANNELS_LANGGRAPH_URL", "http://gateway:9193/api")
        monkeypatch.setenv("KKOCLAW_CHANNELS_GATEWAY_URL", "http://gateway:9193")

        service = ChannelService(channels_config={})

        assert service.manager._langgraph_url == "http://gateway:9193/api"
        assert service.manager._gateway_url == "http://gateway:9193"
    ...
    def test_config_service_urls_override_env(self, monkeypatch):
        ...
        service = ChannelService(
            channels_config={
                "langgraph_url": "http://custom-gateway:9193/api",
                "gateway_url": "http://custom-gateway:9193",
            }
        )

        assert service.manager._langgraph_url == "http://custom-gateway:9193/api"
        assert service.manager._gateway_url == "http://custom-gateway:9193"
    ...
    def test_from_app_config_uses_explicit_config(self):
        ...
        with patch("kkoclaw.config.app_config.get_app_config", side_effect=AssertionError("should not read global config")):
