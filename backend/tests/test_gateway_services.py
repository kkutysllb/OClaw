...
def test_build_run_config_coding_agent_injects_coding_work_mode():
    """Legacy ``coding_agent`` assistant id auto-selects the coding work mode.

    Since the standalone coding_agent graph was retired, the coding tool
    set / CodingThreadState / qiongqi prompt are activated by
    ``work_mode_id="coding"`` inside the unified Lead Agent. Legacy threads
    that still send ``assistant_id="coding_agent"`` must keep getting the
    coding branch, so build_run_config rewrites the mode flag on their
    behalf.
    """
    from app.gateway.services import build_run_config
    ...
    assert config["configurable"]["work_mode_id"] == "coding"


def test_build_run_config_context_coding_agent_injects_coding_work_mode():
    """Legacy ``coding_agent`` context-mode requests also carry the coding mode.

    Same contract as the configurable variant, but for LangGraph >= 0.6.0
    requests that use the ``context`` container.
    """
    from app.gateway.services import build_run_config
    ...
    assert config["context"]["work_mode_id"] == "coding"
