...
def _mock_resolve(artifact_path: Path):
    """Return an async mock matching resolve_thread_artifact_path_async's signature."""

    async def _resolve(_thread_id, _path, _request):
...
def test_get_artifact_reads_utf8_text_file_on_windows_locale(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(artifacts_router, "resolve_thread_artifact_path_async", _mock_resolve(artifact_path))

    request = _make_request()
    response = asyncio.run(call_unwrapped(artifacts_router.get_artifact, "thread-1", str(artifact_path), request))
...
def test_get_artifact_forces_download_for_active_content(tmp_path, monkeypatch, filename: str, content: str) -> None:
    ...
    response = asyncio.run(call_unwrapped(artifacts_router.get_artifact, "thread-1", str(artifact_path), _make_request()))
...
def test_get_artifact_forces_download_for_active_content_in_skill_archive(tmp_path, monkeypatch, filename: str, content: str) -> None:
    ...
    monkeypatch.setattr(artifacts_router, "resolve_thread_artifact_path_async", _mock_resolve(skill_path))

    response = asyncio.run(call_unwrapped(artifacts_router.get_artifact, "thread-1", f"{skill_path}/{filename}", _make_request()))
...
def test_get_artifact_download_false_does_not_force_attachment(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        response = client.get(f"/api/threads/thread-1/artifacts/{str(artifact_path).lstrip('/')}?download=false")
...
def test_get_artifact_download_true_forces_attachment_for_skill_archive(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        response = client.get(f"/api/threads/thread-1/artifacts/{str(skill_path).lstrip('/')}/notes.txt?download=true")
