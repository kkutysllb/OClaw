...
def test_coding_engine_builds_isolated_runtime_context(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kkoclaw.coding_core.skills._builtin_coding_skills_root", lambda: None)
...
def test_coding_skill_registry_loads_builtin_public_coding_skills(tmp_path, monkeypatch):
    ...
    builtin_skill = skills_root / "builtin" / "coding" / "implement"
    ...
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setattr(
        "kkoclaw.coding_core.skills._builtin_coding_skills_root",
        lambda: skills_root / "builtin" / "coding",
    )
...
def test_coding_builtin_public_skills_include_core_and_meta_engineering_skills(monkeypatch):
    ...
    skills = CodingSkillRegistry.discover(project_root=None)
    builtin_ids = [skill.id for skill in skills if "/skills/builtin/coding/" in str(skill.skill_file)]
